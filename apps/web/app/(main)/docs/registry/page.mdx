...
`@json-render/react-email` uses `defineRegistry` like React. Components render to React Email primitives (`@react-email/components`). Use `renderToHtml` or `renderToPlainText` for server-side email output:
