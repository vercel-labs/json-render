...
export const builtInValidationFunctions: Record<string, ValidationFunction> = {
  ...
  minLength: (value: unknown, args?: Record<string, unknown>) => {
    ...
    if (length === undefined) {
      console.warn('minLength validation requires a length argument');
      return false;
    }
  ...
  maxLength: (value: unknown, args?: Record<string, unknown>) => {
    ...
    const length = args?.length;
    if (length === undefined) {
      console.warn('maxLength validation requires a length argument');
      return false;
    }
    if (typeof length !== 'number') return false;
    return value.length <= length;
