 ...
 *   h('view', null, [props.title, children])
 * ```
 */
 ...
 *   return h('button', { onTap: () => ctx.emit("tap") }, ctx.props.label)
 * }
 */
...
/**
 * Component render function type for UniApp
 * @example
 * const Button: ComponentFn<typeof catalog, 'Button'> = ({ props, emit }) =>
 *   h('button', { onTap: () => emit("tap") }, props.label)
 */
