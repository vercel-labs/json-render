  ...
  setup(props: ElementRendererInternalProps) {
    ...
    return () => {
      ...
      const RenderedComponent =
        props.registry[resolvedElement.type] ?? props.fallback;
 ...
 *   components: {
 *     Card: ({ props, children }) => h('view', { class: 'card' }, [props.title, children]),
 *   },
 * });
 *
 * // Both
 ...
 *   Card: ({ props, children }) => h('view', { class: 'card' }, children),
 *   Metric: ({ props }) => h('text', null, props.value),
 * });
 *
 * // Usage in template
